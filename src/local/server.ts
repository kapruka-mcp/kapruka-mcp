        ...
        try {
          ...
          const text = JSON.stringify({
            ...
            note: result.available
              ...
              : '⚠️ Perishable products cannot be delivered to this city due to the distance.',
          });
      ...
      async ({ order_number }) => {
        ...
        try {
          ...
          return this.textContent(JSON.stringify({
            orderId: result.id,
            status: result.status,
            statusLabel: `${STATUS_EMOJI[result.status] ?? '📦'} ${result.status.charAt(0).toUpperCase() + result.status.slice(1)}`,
            items: result.items,
            total: `LKR ${result.total.toLocaleString()}`,
            currency: result.currency,
